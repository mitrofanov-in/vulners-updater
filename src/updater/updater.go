		...
		for _, item_med := range u_med.([]interface{}) {
			...
			for _, item := range u_med.([]interface{}) {
				...
				for _, item := range u_vulndetail.([]interface{}) {
					...
					reInstPack := regexp.MustCompile("(?m)[\r\n]+^.*Installed.*$")
					reFixVer := regexp.MustCompile("(?m)[\r\n]+^.*Fixed version.*$")
					rePath := regexp.MustCompile("(?m)[\r\n]+^.*Path.*$")
					resHtml := reHtml.ReplaceAllString(PlugTextW, "")
					resInstPack := reInstPack.ReplaceAllString(resHtml, "")
					...
					res6 := strings.ReplaceAll(resSpc, "_", " ")
